...
template <typename C> void printVector(const C& v);     // 단순 컨테이너 출력용
...
template <typename C> void bubble(C& v, bool ascending = true) {
    ...
    for (uint i = 0; i < sz - 1; ++i) {
        for (uint j = 0; j < sz - i - 1; ++j)
            if (condition(v[j], v[j + 1], ascending))           swap(v[j], v[j + 1]);
    }
}
...
template <typename C> void select(C& v, bool ascending = true)  {
    ...
    for (uint i = 0; i < sz - 1; ++i) {
        ...
        for (uint j = i; j < sz; ++j)
            if (condition(v[targetIdx], v[j], ascending))       targetIdx = j;      // 비교 만족시 인덱스 업데이트
        if (targetIdx != i)     swap(v[i], v[targetIdx]);       // 업데이트 발생시 swap
    }
}

// 삽입정렬 : 뽑은위치(i) 이전까지의 배열에서만 비교. 비교조건만족시 직전idx를 현재 idx로 밀어내기(swap사용안함) 후 삽입
// 특징: 0번은 정렬된 상태로 보고 1번 idx부터 시작함
...
template <typename T> bool condition(const T& a, const T& b, bool ascending) {
