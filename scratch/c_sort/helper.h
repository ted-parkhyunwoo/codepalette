...
void delete_int_arr(int_arr* iArr) { free(iArr->array); }
...
static int compare_for_qsort(const void* a, const void*b) { return *(const int*)a - *(const int*)b; }
...
static void is_sort_work_correctly(void (*sortFunc)(int*, int*)) {
    ...
    int res = 1;    // boolean. validate sort.
    for (unsigned i = 0; i < size; ++i)
